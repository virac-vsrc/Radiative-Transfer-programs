...
data_Q_1 = init_2d_list('data_gnu_Q_2.txt')
data_U_1 = init_2d_list('data_gnu_U_2.txt')

compare_2d_list = [[0 for x in range(image_x_dim)] for y in range(image_y_dim)]

for y in range(image_y_dim):
   for x in range(image_x_dim):
      if(image_data_2d_list[y][x] != 0 and data_I_1[y][x] != 0 and \
      math.sqrt(data_Q2_list[y][x]**2 + data_U2_list[y][x]**2) !=  0 \
      and math.sqrt(data_Q_1[y][x]**2 + data_U_1[y][x]**2) != 0):
         compare_2d_list[y][x] = (math.sqrt(data_Q2_list[y][x]**2 + data_U2_list[y][x]**2) / image_data_2d_list[y][x] ) / \
                                 (math.sqrt(data_Q_1[y][x]**2 + data_U_1[y][x]**2) / data_I_1[y][x] )
...
matplotlib.pyplot.title('Polarization deg., opt_rad=0.1, albedo=0.99')
str_average_val = '1.018' + u'\u00B1' + '0.011'

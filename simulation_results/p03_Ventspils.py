...
image_data_2d_list = init_2d_list('data_gnu_I.txt')
image_data_2d_list = np.array(image_data_2d_list)*4.997579e-01
...
data_Q2_list = init_2d_list('data_gnu_Q.txt')
data_U2_list = init_2d_list('data_gnu_U.txt')
data_Q2_list = np.array(data_Q2_list)*4.997579e-01
data_U2_list = np.array(data_U2_list)*4.997579e-01
...
matplotlib.pyplot.title('Ventspils, opt_rad=0.1, albedo=0.99')

max_pol_degree = 0 # maximum value of polarization degree

for y in range(int(image_y_dim/cell_size) ):
    for x in range(int(image_x_dim/cell_size) ):
        if(data_for_vectors[y][x] > 0):
...
print('max_pol_degree = ', max_pol_degree)

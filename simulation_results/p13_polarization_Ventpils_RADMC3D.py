...
for y in range(image_y_dim):
   for x in range(image_x_dim):
      if(image_data_2d_list[y][x] > 1e-40 and data_I_1[y][x] != 0 and \
      math.sqrt(data_Q2_list[y][x]**2 + data_U2_list[y][x]**2) !=  0 and \
      (math.sqrt(data_Q_1[y][x]**2 + data_U_1[y][x]**2) / data_I_1[y][x] ) / \
      (math.sqrt(data_Q2_list[y][x]**2 + data_U2_list[y][x]**2) / image_data_2d_list[y][x] ) < 10 ):
         compare_2d_list[y][x] = (math.sqrt(data_Q_1[y][x]**2 + data_U_1[y][x]**2) / data_I_1[y][x] ) / \
            (math.sqrt(data_Q2_list[y][x]**2 + data_U2_list[y][x]**2) / image_data_2d_list[y][x] )
...
my_colorbar.set_label('Ventspils / RADMC3D')
ax = my_colorbar.ax
text = ax.yaxis.label
